...
def plot_and_record_daily(taxi_data, start_date, end_date):
	wb = xlsxwriter.Workbook(os.path.join(output_path, '_'.join([ '_'.join(taxi_data.keys()), str(distance), start_date, end_date ])), {'constant_memory': True})
	...
	for idx, date in enumerate(daterange(start_date, end_date)):
		...
		directory = '_'.join([ '_'.join(taxi_data.keys()), str(distance), datetime.combine(date, datetime.min.time()), datetime.combine(date, datetime.max.time()) ])
		...
		print '\n...Writing this day\'s (' + str(date) + ') satisfying coordinates to file.'

		# Writing this day's satisfying coordinates to a .csv file
		ws.append(combined_coords)
