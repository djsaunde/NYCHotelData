...
plots_path = os.path.join('..', 'plots', 'distance_histograms',
						'_'.join([coord_type, str(distance)]))
...
df = pd.read_csv(fname); print('Time: %.4f\n' % (timeit.default_timer() - start))

df['Hotel Name'] = df['Hotel Name'].apply(str.strip)

start = timeit.default_timer()
for hotel in df['Hotel Name'].unique():
	if os.path.isfile(os.path.join(plots_path, hotel.replace('.', '') + '.png')):
		continue

	print('... Plotting distance histogram for %s (Time: %.4f)' % (hotel, (timeit.default_timer() - start)))

	df[df['Hotel Name'] == hotel]['Distance From Hotel'].hist(bins=np.arange(0, distance + 2, 1))
	plt.title('Binned distances by feet (%s)' % hotel)
	plt.xlabel('Distance in feet'); plt.ylabel('No. observations')
	plt.legend(); plt.tight_layout()
	plt.savefig(os.path.join(plots_path, hotel.replace('.', '')))
...
df['Distance From Hotel'].hist(bins=np.arange(0, distance + 2, 1));
plt.title('Binned distances by feet (all hotels)')
plt.xlabel('Distance in feet'); plt.ylabel('No. observations')
plt.legend(); plt.tight_layout()
plt.savefig(os.path.join(plots_path, 'All hotels'))
