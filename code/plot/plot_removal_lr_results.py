...
parser.add_argument('--n_removals', type=int, default=25)
...
for d, c in zip(distances, colors):
	...
	axes[0].semilogy(results['Test MSE'][:n_removals + 1], c=c, label='d = %d' % d)
	axes[1].plot(results['Test R^2'][:n_removals + 1], c=c, label='d = %d' % d)
...
# Plot the test mean-squared errors.
axes[0].semilogy(results['Test MSE'][:n_removals + 1], c='k', linestyle='--', label='Naive')
axes[1].plot(results['Test R^2'][:n_removals + 1], c='k', linestyle='--', label='Naive')
...
axes[0].set_xticks(range(n_removals + 1)); axes[1].set_xticks(range(n_removals + 1))
axes[0].legend(fontsize='x-small'); axes[1].legend(fontsize='x-small');
plt.tight_layout()

plt.savefig(os.path.join(plots_path, 'taxi_lr_removals_results.png'))
plt.show()