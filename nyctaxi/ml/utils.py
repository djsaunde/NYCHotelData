...
def load_merged_data(data_path, taxi_occupancy_path, preproc_data_path, start_date, end_date, trip_type):
    ...
    if not is_counts_file and not is_data_file:
        ...
        taxi_rides = taxi_rides.loc[
            (taxi_rides['Pick-up Time'] >= start_date) & (taxi_rides['Drop-off Time'] <= end_date)
        ]
        ...
        # Make sure occupancy is in the merged occupancy and taxi data.
        if 'Occ' not in df.columns:
            ...
            occupancy['Occ'] = occupancy['Occ'] / 100
            occupancy = occupancy.rename(index=str, columns={'Share ID': 'Hotel Name'})
            occupancy = occupancy[['Date', 'Hotel Name', 'Occ']]

            df = pd.merge(occupancy, df, on=['Date', 'Hotel Name'])
...
def encode_data(df, obs=('IDs', 'Weekdays', 'Months', 'Years'), targets=('Occ', )):
    ...
    if 'No. Nearby Trips' in df:
        ...
        observations = observations + [trips]
